...
public class EnumOptionUI<E extends Enum<E>> extends JComboBox implements FormOptionUI {
	private static final long serialVersionUID = 1L;
	private EnumOption<E> o;
	private UpdateLock u = new UpdateLock();
	
	public EnumOptionUI(EnumOption<E> opt, boolean mini) {
		this.o = opt;
		E sel = o.getValue();
		int selidx = 0;
		final E[] enums = o.values();
		final String[] strings = new String[enums.length];
		for (int i = 0; i < enums.length; i++) {
			strings[i] = o.getLabel(enums[i]);
			if (enums[i] == sel) selidx = i;
		}
		...
		addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (u.lock()) {
					o.setValue(enums[getSelectedIndex()]);
					u.unlock();
				}
			}
		});
