...
public class DoubleOptionUI extends JSpinner implements FormOptionUI, FormMetrics {
	private static final long serialVersionUID = 1L;
	private SpinnerModel sm;
	private DoubleOption o;
	private boolean m;
	private UpdateLock u = new UpdateLock();
	
	public DoubleOptionUI(DoubleOption opt, boolean mini) {
		this.o = opt;
		this.m = mini;
		...
		addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				if (u.lock()) {
					o.setValue(((Number) sm.getValue()).doubleValue());
					u.unlock();
				}
			}
		});
