...
public abstract class RGBFilter extends AbstractFilter {
	...
	public final Image filter(Image src) {
		...
		for (int y = 0, py = 0; y < h && py < pixels.length; y++, py += w) {
			for (int x = 0, px = py; x < w && px < pixels.length; x++, px++) {
				pixels[px] = filterRGB(x, y, pixels[px]);
			}
		}
