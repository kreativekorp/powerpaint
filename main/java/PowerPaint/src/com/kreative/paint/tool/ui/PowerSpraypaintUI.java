...
public class PowerSpraypaintUI extends JPanel implements ToolContextListener {
	private static final long serialVersionUID = 1L;
	private ToolContext tc;
	private FormUI fui;
	private Set<PowerBrushLabel> brushLabels;
	
	public PowerSpraypaintUI(ToolContext tc, boolean mini) {
		this.tc = tc;
		Form f = new Form();
		f.add(new DoubleOption() {
			public String getName() { return ToolUtilities.messages.getString("options.Width"); }
			public double getMaximum() { return Float.MAX_VALUE; }
			public double getMinimum() { return 1.0f; }
			public double getStep() { return 1.0f; }
			public double getValue() { return PowerSpraypaintUI.this.tc.getPowerBrushOuterWidth(); }
			public void setValue(double v) {
				PowerSpraypaintUI.this.tc.setPowerBrushOuterWidth((float)v);
				PowerSpraypaintUI.this.tc.setPowerBrushInnerWidth((float)v);
				PowerSpraypaintUI.this.tc.setPowerBrushInnerHeight(PowerSpraypaintUI.this.tc.getPowerBrushOuterHeight());
			}
		});
		f.add(new DoubleOption() {
			public String getName() { return ToolUtilities.messages.getString("options.Height"); }
			public double getMaximum() { return Float.MAX_VALUE; }
			public double getMinimum() { return 1.0f; }
			public double getStep() { return 1.0f; }
			public double getValue() { return PowerSpraypaintUI.this.tc.getPowerBrushOuterHeight(); }
			public void setValue(double v) {
				PowerSpraypaintUI.this.tc.setPowerBrushOuterHeight((float)v);
				PowerSpraypaintUI.this.tc.setPowerBrushInnerHeight((float)v);
				PowerSpraypaintUI.this.tc.setPowerBrushInnerWidth(PowerSpraypaintUI.this.tc.getPowerBrushOuterWidth());
			}
		});
		f.add(new IntegerOption() {
			public String getName() { return ToolUtilities.messages.getString("options.FlowRate"); }
			public int getMaximum() { return Integer.MAX_VALUE; }
			public int getMinimum() { return 1; }
			public int getStep() { return 1; }
			public int getValue() { return PowerSpraypaintUI.this.tc.getPowerBrushFlowRate(); }
			public void setValue(int v) { PowerSpraypaintUI.this.tc.setPowerBrushFlowRate(v); }
		...
		for (final BrushShape bs : BrushSettings.SHAPES) {
			...
			l.setSelected(PowerSpraypaintUI.this.tc.getPowerBrushShape() == bs);
			l.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					PowerSpraypaintUI.this.tc.setPowerBrushShape(bs);
					for (PowerBrushLabel ll : brushLabels) {
						ll.setSelected(PowerSpraypaintUI.this.tc.getPowerBrushShape() == ll.getBrushShape());
					}
				}
			});
			brushLabels.add(l);
