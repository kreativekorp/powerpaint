...
public class SpriteIntent {
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			for (String arg : args) {
				int intent = fromString(arg);
				System.out.println(intent + "\t" + toString(intent));
			}
		} else {
			for (int i = 0; i < 0x100; i++) {
				String intString = Integer.toString(i);
				String intentString = toString(i);
				if (!intString.equals(intentString)) {
					System.out.println(intString + "\t" + intentString);
				}
				assert(fromString(intString) == i);
				assert(fromString("0x" + Integer.toHexString(i)) == i);
				assert(fromString(intentString) == i);
			}
		}
	}
}
